...
bool SystemExec(const char *szFormatCmd, ...)
{
    ...
    // On iOS, system() is not available, so we skip system commands
    ZLog::WarnV("SystemExec: system() not available on iOS, skipping command: %s\n", szCmd);
    return true; // Return true to avoid breaking the signing flow
#else
...
{
    PrintSHASum(prefix, (const uint8_t *)strSHASum.data(), (uint32_t)strSHASum.size(), suffix);
}
